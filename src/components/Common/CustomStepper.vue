      ...
      <template v-for="(step, index) in steps">
        ...
        <b-col :xs="responsive.xs" :xxs="responsive.xxs" class="position-relative stepper-col">
          ...
          <div @click="!disabled && handleStepClick(index)" class="cursor-pointer stepper-item p-3" :class="{
            ...
            'stepper-item-rejected': (index == 3 && requestStatus === 'SUPERVISION_REJECTED'),
            'stepper-item-disabled': disabled
          }">
            <div class="stepper-number mb-2"
              :class="(index == 3 && requestStatus === 'SUPERVISION_REJECTED') ? 'stepper-number-rejected' : ''">
              {{ index + 1 }}
            </div>
            ...
            <div v-if="step.description && showDescription" class="stepper-description mt-1">
              {{ step.description }}
            </div>
          </div>
        </b-col>
...
export default {
  ...
  computed: {
    ...mapGetters(["requestStatus", "stateCandidInfo"]),
    progressPercentage() {
