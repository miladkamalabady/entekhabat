<template>
<div>    <div class="glide"  ref="carouselImages" >
        <div data-glide-el="track" class="glide__track">
            <div class="glide__slides">
                <slot></slot>
            </div>
        </div>
        <div v-if="!settings.hideNav" :class="currentUser?.result?.student?.gradeTypeId < 7
                  ? 'a18d4ff'
                  : currentUser?.result?.student?.gradeTypeId < 10
                  ? 'a0061f2'
                  : currentUser?.result?.student?.gradeTypeId
                  ? 'a21928A'
                  : ''" class="glide__arrows slider-nav" data-glide-el="controls">
            <button v-if="!settings?.shownavcircle" class="glide__arrow glide__arrow--left left-arrow btn btn-link" data-glide-dir="<">
            ...
            <button v-if="!settings?.shownavcircle" class="glide__arrow glide__arrow--right right-arrow btn btn-link" data-glide-dir=">">
    ...
    methods: {
        initGlide() {
          this.selfWidth = this.$el.offsetWidth;
            ...
            if(this.settings?.shownavcircle)
