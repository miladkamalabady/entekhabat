...
if(location.origin=='http://localhost:8081')
 baseurl='http://172.16.51.19:5002';
function GetCaptcha() {
  $.ajax({
    type: "GET",
    headers: {
  ...
  $.ajax({
    ...
    success: function (responce) {
      ...
      responce.data.sliders.forEach(element => {
        if(temp=='')
          temp+='<div class="carousel-item active " style="min-height: 500px;scale: 0.7;height: 500px;display: flex;"><img class="d-block" loading="lazy" src="'+element.imageUrl+'" alt="my medu" style="max-width: 100%;margin: auto;"></div>'
          else
          temp+='<div class="carousel-item " style="min-height: 500px;scale: 0.7;height: 500px;display: flex;"><img class="d-block" loading="lazy" src="'+element.imageUrl+'" alt="my medu" style="max-width: 100%;margin: auto;"></div>'
        });
      ...
      temp+='<td class="text-center text-nowrap"><span style="font-size: 0.7rem"></span></td><td class="text-center" style="font-size: 0.7rem"><span>'+element.presenter+'</span></td>';
    else
    temp+='<td class="text-center text-nowrap"><span style="font-size: 0.7rem">'+element.pEndtDate+'</span></td><td class="text-center" style="font-size: 0.7rem"><span>'+element.presenter+'</span></td>';
      temp+='</tr>';
    });
    $(function () {
...
function isNumber(evt) {
    ...
    else if ((charCode > 31 && (charCode < 48 || charCode > 57) && !(charCode < 106 || charCode > 96)) && charCode !== 46 ) {
      evt.preventDefault();
    } else {
      return true;
    }
...
function gotosso(mode) {
    ...
    if (this.currentUser) {
      if (mode == 1) {
        window.location.href = this.returnRedirectUrl(
