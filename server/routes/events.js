...
router.get('/', async (req, res) => {
  try {
    const [rows] = await db.query('SELECT id, json_data FROM events ORDER BY id ASC')
...
router.post('/', async (req, res) => {
  try {
    ...
    await db.query(
      'INSERT INTO events (id, json_data) VALUES (?, ?)',
...
router.delete('/:id', async (req, res) => {
  try {
    await db.query('DELETE FROM events WHERE id = ?', [Number(req.params.id)])
    res.json({ ok: true })
  } catch (e) {
    res.status(500).json({ error: e.message })
  }
