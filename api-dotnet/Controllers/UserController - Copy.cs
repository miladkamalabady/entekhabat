    ...
    [FromQuery] string? search = null)
{
    ...
    if (isProvinceSupervisor)
    {
        ...
        param.Add("provinceCode", (int)currentUser.ProvinceCode);
    }
    ...
    if (lastId.HasValue)
    {
        conditions.Add("u.id < @lastId");
        param.Add("lastId", lastId.Value);
    }
    ...
    string passColumns = isAdmin
        ? @",
            CASE
                WHEN u.roles='EXECUTIVE'
                     AND (u.region_id % 100) != 0
                THEN fra.EXECUTIVEPass
                ELSE NULL
            END AS executivePass,
            CASE
                WHEN u.roles='SUPERVISOR'
                     AND (u.region_id % 100) != 0
                THEN fra.SUPERVISORPass
                ELSE NULL
            END AS supervisorPass"
        : "";
    ...
    long? nextLastId = list.Count > 0
        ? Convert.ToInt64(((IDictionary<string, object>)list.Last())["id"])
        : null;
